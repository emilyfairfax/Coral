...
for i=1:imax
    ...
    sealevel(1:N) = SeaLevelFunction(t(i),ampsealevel,period); %name the function, define inputs; % sea level oscillation in meters relative to mean sea level
    ...
    dcoral = CoralFunction(Gm,I,Ik,k,absdepth,dt); % change in coral growth in meters
...
% Plot results
    ...
    sea = find(sealevel(i)>coralsl); %find only where it doesnt overlap the coral or bedrock
   
    
    figure(1)
    clf
    
    plot(x,coralsl,'m','linewidth',3) %color the coral line pink
    hold all
