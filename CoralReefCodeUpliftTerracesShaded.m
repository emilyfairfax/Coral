...
% Plot results
    ...
    sea = find(sealevel(i)>coralsl); %find only where the water does not overlap the bedrock or coral
    ...
    plot(x(sea),waterlevel(sea),'b','linewidth',3) %draw waterline in blue only where not overlapping bedrock or coral
